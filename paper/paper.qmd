...
A major component of this project involved minimizing the moving average calculation to a single date before applying the process over the entire range of sample dates. A single 
...
plot_facet <- qb_mpr_ma %>% mutate(year_sample = year(Sample_Date)) %>%
   ...
   # drop_na(moving_avg_nutrients) %>%  
    ...
    labs(title = "Nine Week Moving Averages of Nutrients/Contaminants (1988-1998)",
         ...
         x = "Year", caption = "Hurricane Hugo (marked by the dashed line) occurred around 09/21/1989.") + facet_wrap(~factor(nutrients), scales = "free_y", ncol = 1) + geom_vline(xintercept = hugo, linetype = "dashed", col = "red") +
  theme(
